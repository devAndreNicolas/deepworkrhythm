      ...
      <div className="mx-auto flex max-w-[1380px] flex-col gap-10">
        ...
        <section className="grid gap-8 lg:grid-cols-[minmax(0,1.45fr)_minmax(330px,0.8fr)]">
          <div className="sketch-panel hero-panel flex min-h-[520px] flex-col justify-between p-6 md:p-10">
            ...
            <div className="flex flex-col items-center gap-7 py-7"><div className="timer-ring" style={{ '--progress': `${progress}%` } as React.CSSProperties}><div className="timer-inner"><span className="font-mono text-6xl tracking-[-0.08em] md:text-8xl">{formatTime(remaining)}</span><span className="eyebrow mt-2">{running ? 'in progress' : remaining === 0 ? 'complete' : 'ready to begin'}</span></div></div><div className="flex items-center gap-3"><button className="icon-button" onClick={reset} aria-label="Reset session"><RotateCcw /></button><button className="primary-action" onClick={() => setRunning(!running)}>{running ? <Pause data-icon="inline-start" /> : <Play data-icon="inline-start" />}{running ? 'Pause' : 'Start'}</button><button className="icon-button" onClick={() => setRemaining((value) => Math.min(duration, value + 60))} aria-label="Add one minute"><ArrowRight /></button></div></div>
